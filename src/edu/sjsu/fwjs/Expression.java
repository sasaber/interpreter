...
class BinOpExpr implements Expression {
    ...
    public Value evaluate(Environment env) {
        ...
        IntVal v1Int = (IntVal) v1;
        IntVal v2Int = (IntVal) v2;

        // apply binary operation
        switch(op) {
            case ADD:
                return new IntVal(v1Int.toInt() + v2Int.toInt());
            case SUBTRACT:
                return new IntVal(v1Int.toInt() - v2Int.toInt());
            case MULTIPLY:
                return new IntVal( v1Int.toInt() * v2Int.toInt());
            ...
            case MOD:
                return new IntVal( v1Int.toInt() % v2Int.toInt());
            case GT:
                return new BoolVal(v1Int.toInt() > v2Int.toInt());
            case GE:
                return new BoolVal(v1Int.toInt() >= v2Int.toInt());
            case LT:
                return new BoolVal(v1Int.toInt() < v2Int.toInt());
...
class IfExpr implements Expression {
    ...
    public Value evaluate(Environment env) {
        // evaluate condition
        try {
            BoolVal bVal = (BoolVal) cond.evaluate(env);
            // if condition is true
            if (bVal.equals(new BoolVal(true))) {
                return thn.evaluate(env);
            } else if (bVal.equals((new BoolVal(false)))){
